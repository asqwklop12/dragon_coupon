...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleCouponNotFound(CouponNotFoundException e) {
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(ApiResponse.failResponse(e.getMessage()));
  }

  @ExceptionHandler(CouponExhaustedException.class)
  public ResponseEntity<ApiResponse<Void>> handleCouponExhausted(CouponExhaustedException e) {
    return ResponseEntity.status(HttpStatus.CONFLICT)
        .body(ApiResponse.failResponse(e.getMessage()));
  }

  @ExceptionHandler(DuplicateCouponIssueException.class)
  public ResponseEntity<ApiResponse<Void>> handleDuplicateIssue(DuplicateCouponIssueException e) {
    return ResponseEntity.status(HttpStatus.CONFLICT)
        .body(ApiResponse.failResponse(e.getMessage()));
  }

  @ExceptionHandler(CouponNotAvailableException.class)
  public ResponseEntity<ApiResponse<Void>> handleNotAvailable(CouponNotAvailableException e) {
